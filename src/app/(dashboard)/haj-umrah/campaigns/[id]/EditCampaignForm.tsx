...
export default function EditCampaignForm({ campaignId, initial, users }: Props) {
  const router = useRouter();
  const [date, setDate] = useState(initial.date);
  const [time, setTime] = useState(initial.time);
  const [name, setName] = useState(initial.name);
  const [type, setType] = useState<"haj" | "umrah" | "">(initial.type);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/haj-umrah/campaigns/${campaignId}`, {
        method: "PATCH",
      ...
      router.push(`/haj-umrah/campaigns/${campaignId}`);
      router.refresh();
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-xl space-y-6 rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="flex gap-3">
        ...
        <Link
          href={`/haj-umrah/campaigns/${campaignId}`}
