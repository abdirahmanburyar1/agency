...
export default function EditVisaForm({ initial }: { initial: InitialData }) {
  ...
  useEffect(() => {
    fetch("/api/settings/ticket-options")
      .then((r) => r.json())
      .then((data) => setOptions({ country: data.country ?? [] }))
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    fetch("/api/customers/for-select")
      ...
      .then((data) => {
        ...
        const hasCurrent =
          initial.customerId &&
          list.some((c: { id: string }) => c.id === initial.customerId);
        if (initial.customerId && initial.customerName && !hasCurrent) {
          const merged = [
            ...list,
            {
      ...
      .catch(() => {
        if (initial.customerId && initial.customerName) {
          setCustomers([
            {
              id: initial.customerId,
              name: initial.customerName,
  ...
  }, [initial.customerId, initial.customerName, initial.customerPhone]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/visas/${initial.visaId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          date,
          month: monthToName(monthValue),
          sponsor: initial.sponsorName || null,
      ...
      if (!res.ok) {
        setError(data.error ?? "Failed to update visa");
        return;
      }
      router.push(`/visas/${initial.visaId}`);
      router.refresh();
    } catch {
          ...
          <div className="sm:col-span-2 flex gap-4">
            <div className="flex-1 flex gap-3">
              <div className="w-[100px] shrink-0">
                ...
                <input
                  type="text"
                  value={initial.visaNumberDisplay}
                  disabled
