...
export default function CreateTicketForm({
  ...
  const [monthValue, setMonthValue] = useState(monthDefault); // YYYY-MM for input type="month"
  ...
  function onDateChange(value: string) {
    setDate(value);
    if (value) setMonthValue(value.slice(0, 7)); // Sync month picker with date
  }

  function onMonthChange(value: string) {
    setMonthValue(value);
    if (value) setDate(value + "-01"); // Sync date to first of month
  }

  // Convert YYYY-MM to "February" for API
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({ form: data.error ?? "Failed to create ticket" });
        return;
      }
      router.push("/tickets");
      router.refresh();
        ...
        <div className="flex flex-col gap-4 sm:col-span-2 sm:flex-row sm:gap-4">
          <div className="flex flex-col gap-4 sm:flex-[3] sm:flex-row sm:gap-4">
            <div className="min-w-0 flex-1">
              ...
              />
              <p className="mt-0.5 text-xs text-zinc-500">Auto-generated</p>
            </div>
            <div className="min-w-0 flex-1">
              <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                Reference *
